...
function App() {
  ...
  useEffect(() => {
    if (route === '/app/schedule' && session?.role === 'EMPLOYEE') {
      navigate('/app');
    }
  }, [route, session]);
  ...
  return (
    ...
    <AppShell
      ...
      onSelfImport={session?.role === 'EMPLOYEE' && session.employeeId ? () => {
        if (!isImporting) setIsImportOpen(true);
      } : undefined}
      onHistoricalAdd={session?.role === 'EMPLOYEE' && session.employeeId ? () => {
        if (isImporting) return;
        setEditingShiftId(null);
        setDraftShiftDate(getPreviousOperationalDate());
        setIsModalOpen(true);
      } : undefined}
      onRequests={session?.role === 'EMPLOYEE' && session.employeeId ? () => {
        if (!isImporting) setIsEmployeeRequestsOpen(true);
      } : undefined}
    ...
    >
      ...
      {session?.role === 'EMPLOYEE' && session.employeeId && (
        <NewChangeRequestModal
          ...
          employeeId={session.employeeId}
          onClose={() => setIsNewEmployeeRequestOpen(false)}
          onCreated={() => {
            setIsNewEmployeeRequestOpen(false);
            setEmployeeRequestRefreshSignal((current) => current + 1);
          }}
        />
      )}
