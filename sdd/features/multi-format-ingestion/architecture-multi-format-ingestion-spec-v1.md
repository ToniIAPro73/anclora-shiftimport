...
- ~~**Perfiles aprendidos son localStorage-only**: sin sync remoto, sin compartir entre usuarios de la org, sin tier global.~~ **RESUELTO (Format Memory v1, 2026-08-26)**: persistencia server-side org-scoped, compartida entre usuarios de la organización. Tier global (Phase 7) sigue sin existir.
- **No existen áreas** ni bindings perfil↔área (ni en DB ni en dominio). *(Nota: `areas` sí existe desde la migración `0008_areas_optional.sql`, anterior a este análisis; `area_profile_bindings` M:N sigue sin existir.)*
- ~~**No hay ciclo de vida de perfiles** (candidate/validated/verified/legacy/deprecated): solo `useCount` y drift.~~ **RESUELTO (Format Memory v1)**: ciclo de vida completo candidate→validated→verified, legacy, deprecated, con reactivación.
...
- **Phase 2 — FormatProfile server-side + lifecycle. COMPLETADA (2026-08-26, ver `sdd/features/format-memory-v1/`).** Migración real `0009_format_profiles.sql` (numeración corregida frente a este documento: la última migración aplicada al iniciar la fase era `0008_areas_optional.sql`) — org-scoped, `signature` JSONB hash-only, `token_aliases`/`parser_config`/`code_times`/`off_tokens`/`day_column_map`/`tabular_memory` JSONB, `status` (candidate/validated/verified/legacy/deprecated), `use_count`/`successful_use_count`, `logical_profile_id` + `version` + `supersedes_profile_id` para versionado drift-safe. API completa en `api/format-profiles/index.js` + `api/_lib/format-profiles.js` (list/get/create-candidate/rename/confirm/deprecate/reactivate/record-use, aislamiento multi-tenant verificado, sanitización allowlist server-side independiente del cliente). Stores cliente `LocalFormatProfileStore`/`RemoteOrganizationFormatProfileStore` (`src/lib/format-profile-store.ts`) sustituyen el acceso directo a `localStorage` en `ImportModal.tsx`/`ProfileAssistantPanel.tsx`; migración explícita local→organización (`FormatProfileMigrationModal.tsx`); UI de gestión "Formatos aprendidos" (`FormatProfilesModal.tsx`). Reutilización automática (`profilesHint` ya existente en `analyzeDocumentFile` fue el punto de integración, sin tocar la lógica de matching/drift). Drift crea candidate v2 sin sobrescribir la versión estable, confirmar promueve y degrada la anterior a `legacy`, nunca se pierde historial. Cero PII verificado por inspección directa de la base de datos real (`qa/e2e-acceptance/format-memory-acceptance.mjs`, 29/29). Global Format Registry (Phase 7) explícitamente NO incluido en esta fase — sigue pendiente.
...
- ~~Política de `resetOrganization` sobre areas/profiles/bindings (¿dato operativo o configuración? — decidir con evidencia cuando existan las tablas y actualizar su docstring).~~ **RESUELTO (Format Memory v1)**: `format_profiles`, igual que `areas`, es configuración organizativa y sobrevive a `resetOrganization` — decisión documentada en `sdd/features/format-memory-v1/01_TECHNICAL_DESIGN.md` y reflejada en el docstring de `resetOrganization` (`api/_lib/data.js`). Bindings área↔perfil (M:N) siguen sin existir, pendiente de Phase 3.
