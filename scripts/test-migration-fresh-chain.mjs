/**
 * scripts/test-migration-fresh-chain.mjs
 *
 * Verifies the complete repository migration chain from scratch in a clean, empty database
 ...
 * 4. Empty database created in the isolated branch.
 * 5. Full sequential run of every repository migration via runMigrations().
 * 6. Verification of continuous ledger records matching repository SHA-256 checksums.
...
const FRESH_DB_NAME = 'fresh_chain_test';
...
async function runFreshChainTest() {
  ...
  const repoFiles = (await readdir(MIGRATIONS_DIR))
    .filter((f) => /^\d{4}_[a-z0-9_]+\.sql$/.test(f))
    .sort();
  const expectedCount = repoFiles.length;
  const latestMigration = repoFiles.at(-1);
  ...
  try {
    ...
    // 5. Apply the full repository migration chain from scratch via runMigrations() [Pass 1]
    console.log(`[test] Running full migration chain through ${latestMigration} [Pass 1]...`);
    ...
    if (result.appliedCount !== expectedCount) {
      throw new Error(`Expected exactly ${expectedCount} applied migrations in Pass 1, got: ${result.appliedCount}`);
    }
    ...
    try {
      ...
      if (status.state !== 'UP_TO_DATE' || status.applied.length !== expectedCount || status.pending.length !== 0) {
        throw new Error(
          `Expected state UP_TO_DATE with ${expectedCount} applied, got: state=${status.state}, applied=${status.applied.length}`
        );
      }

      // Verify all continuous ledger entries
      ...
      if (rows.length !== expectedCount) {
        throw new Error(`Expected ${expectedCount} rows in _migrations, found: ${rows.length}`);
      }
      ...
      console.log(`[test] PASS: All ${expectedCount} continuous ledger records match exact repository file checksums.`);
    ...
    console.log('\n======================================================');
    console.log(`ALL FRESH CHAIN THROUGH ${latestMigration} MIGRATION TESTS PASSED!`);
    console.log('======================================================\n');
  } finally {
    if (ephemeralInfo) {
