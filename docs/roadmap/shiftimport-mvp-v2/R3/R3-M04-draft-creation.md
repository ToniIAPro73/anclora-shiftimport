...
MISSING. Depende de R3-M01/M02/M03 (schema completo).
...
Sigue el patrón de rutas existente (`api/employees/index.js`, `api/imports/index.js`): archivo nuevo `api/schedules/index.js` con handler por método HTTP. Reutiliza `api/_lib/auth.js` para sesión + rol, y una nueva función en `api/_lib/data.js` (o un módulo separado `api/_lib/scheduling.js` si R0-M05 decidió dividir `data.js` por dominio — seguir esa decisión).
...
## 9. API / Backend
`POST /api/schedules`
Body: `{ areaId?: number, periodStart: string(ISO date, lunes) }`
...
## 12. i18n
Mensajes de error (409, 403) con claves i18n ES/EN, siguiendo `src/lib/i18n.ts`.
...
- [ ] Segunda llamada con mismo org+area+period reutiliza el `Schedule` existente.
- [ ] Llamada mientras hay DRAFT activo devuelve 409 con mensaje claro.
- [ ] Usuario sin rol PLANNER+ recibe 403.
...
## 20. Evidencias
Endpoint commiteado, tests en PASS, ejemplo de respuesta 201 y 409 documentados.
...
## 23. Criterio de DONE
Endpoint operativo, guards verificados por test, Gate G4+G5+G10 PASS.
