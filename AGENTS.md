...
- `api/`: Vercel Functions multi-tenant sobre Neon (Fase 1). `_lib/auth.js` (sesiones cookie + contexto de seguridad multi-org sin fallback silencioso), `_lib/data.js` (data-access org-scoped, tests con sql fake), `auth/`, `session/`, `employees/`, `imports/`, `shifts/`, `memberships/` (gestión B2B mínima, solo ADMIN). Aislamiento por `organization_id` forzado en backend, nunca en frontend.
- `src/components/AuthScreen.tsx`: pantalla de login contractual (ANCLORA_AUTH_LOGIN_SCREEN_CONTRACT v1.3.0). `src/components/ui/ModalShell.tsx`: primitive modal común (ESC, click-outside, focus trap, ARIA, modo blocking) para modales nuevos.
